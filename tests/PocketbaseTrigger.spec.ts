...
describe('PocketbaseTrigger', () => {
	...
	it('should perform handshake on PB_CONNECT', async () => {
		...
		// Simulate PB_CONNECT
		const pbConnectCallback = esInstance.addEventListener.mock.calls.find((call: any) => call[0] === 'PB_CONNECT')[1];
		await pbConnectCallback({ data: JSON.stringify({ clientId: 'mock-client-id' }) });

		expect(triggerFunctions.helpers.requestWithAuthentication).toHaveBeenCalledWith('pocketbaseHttpApi', {
			method: 'POST',
	...
	it('should emit data on matching event', async () => {
		...
		const collectionCallback = esInstance.addEventListener.mock.calls.find((call: any) => call[0] === 'posts')[1];
