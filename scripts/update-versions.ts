...
async function getLatestPocketbaseVersion() {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch PocketBase version from ${url}: ${response.status} ${response.statusText}`,
    );
  }
...
async function updateDockerCompose(
  ...
  if (pbMatch) {
    ...
    throw new Error(
      `Could not find expected pocketbase image in docker-compose.test.yml: ${pbRegex.source}`,
    );
  }
  ...
  if (n8nMatch) {
    ...
    throw new Error(
      `Could not find expected n8n image in docker-compose.test.yml: ${n8nRegex.source}`,
    );
  }

  // Update symlink path in entrypoint: .../node_modules/old-name
  const symlinkRegex =
    /(ln -sf \/home\/node\/custom-nodes \/home\/node\/.n8n\/nodes\/node_modules\/)\s*([^\s&]+)/;
  const symlinkMatch = content.match(symlinkRegex);
  if (symlinkMatch && symlinkMatch[2] !== packageName) {
    console.log(
      `docker-compose.test.yml: symlink path (.../node_modules/${symlinkMatch[2]} -> .../node_modules/${packageName})`,
    );
...
async function updateIntegrationTest(packageName: string, dryRun: boolean) {
  ...
  for (const match of matches) {
    if (match[1] !== packageName) {
      console.log(
        `integration_test.json: node type (${match[1]}.pocketbaseHttp -> ${packageName}.pocketbaseHttp)`,
      );
      content = content.replaceAll(
        `"type": "${match[1]}.pocketbaseHttp"`,
        `"type": "${packageName}.pocketbaseHttp"`,
      );
      updated = true;
      break;
    }
...
async function updateNodeJson(packageName: string, dryRun: boolean) {
  ...
  const updatedContent = originalContent.replace(urlRegex, (match, org, repo, anchor) => {
    if (
      repo !== packageName &&
      (repo === "n8n-nodes-pocketbase" || repo.startsWith("n8n-nodes-"))
    ) {
...
async function main() {
  try {
    ...
    const anyUpdated =
      packageUpdated || readmeUpdated || dockerUpdated || testUpdated || nodeJsonUpdated;
